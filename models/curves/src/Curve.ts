...
class Curve<T> {
    ...
    constructor(keys: Keyframe<T>[] = [], modifiers: CurveModifier<T>[] = [], endBehavior: EndBehavior = EndBehavior.Clamp, smoothing: number = 0.25) {
        this.keys = keys;
    ...
    removeKeyframe(time: number): void {
        let index = this.keys.findIndex(k => k.time == time);
    ...
    evaluate(time: number): T {
        let rawValue = this._rawEvaluate(time);
        return this._applyModifiers(rawValue, time);
    }

    protected _applyModifiers(rawValue: T, time: number): T {
        let value = rawValue;

        // Apply each modifier
        this.modifiers.forEach((m: CurveModifier<T>) => {
